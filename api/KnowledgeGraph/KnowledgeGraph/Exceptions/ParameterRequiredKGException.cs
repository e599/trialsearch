...
namespace KnowledgeGraph.Exceptions
{
    ...
    /// </summary>
    public class ParameterRequiredKGException : KGException
    {
        private const int c_iErrorNumber = 2;
        ...
        public ParameterRequiredKGException(string parameterName, string parameterValue) : base() {
        ...
        public override void LogError(ILogger<KGException> logger) {
            logger.LogInformation(c_iErrorNumber, $"{c_iErrorNumber}: ParameterName=[{m_sParameterName}] is required. ParameterValue=[{m_sParameterValue}] was provided.");
        }
    }
}
